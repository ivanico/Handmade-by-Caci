...
- Toast system (no external library): fixed top-right, auto-dismiss 3s, variants: success/error/info
...
Type=simple
User=shop
WorkingDirectory=/srv/shop/backend
EnvironmentFile=/srv/shop/backend/.env
ExecStart=/srv/shop/backend/.venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000
Restart=on-failure
RestartSec=5
...
Type=simple
User=shop
WorkingDirectory=/srv/shop/frontend
